---
layout: single
title: "[Pandas] Basic - 데이터 분석"
categories: ['Basic']
tags: ['python', 'pandas']
---

### Pandas library (PANel DAta)
- 지난 시간에 배운 내용을 바탕으로 실제 데이터에 적용하여 간단한 데이터 분석을 해본다.
...
- max(axis=1) 함수는 각 row 당 최대값을 보여준다. 
...
- min(axis=1) 함수는 각 row 당 최값을 보여준다. 
...
#### apply 변환
3) 행, 열 단위로 더 복잡한 처리를 하고 싶을 때 사용
